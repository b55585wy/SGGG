...
export default function PreviewPage() {
  ...
  if (!draft) return <div className="flex items-center justify-center min-h-[60dvh]"><div className="w-8 h-8 border-2 border-[var(--color-accent)]/30 border-t-[var(--color-accent)] rounded-full animate-spin" /></div>;
  ...
  return (
    <div className="pb-12">
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={spring} className="mb-6">
        <div className="flex items-center gap-2 mb-2"><BookOpen size={20} weight="fill" className="text-[var(--color-accent)]" /><span className="text-xs font-mono font-medium tracking-wider text-[var(--color-muted)] uppercase">Story Preview</span></div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.05 }} className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border-light)] p-5 mb-4" style={{ boxShadow: '0 20px 40px -15px rgba(0,0,0,0.03)' }}>
        <p className="text-sm leading-relaxed">{m.summary}</p>
      </motion.div>

      {/* Design Logic — prominently displayed */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.1 }} className="bg-[var(--color-accent-light)] rounded-2xl border border-[var(--color-accent)]/20 p-5 mb-4">
        <div className="flex items-center gap-2 mb-3"><Lightbulb size={18} weight="fill" className="text-[var(--color-accent)]" /><span className="text-xs font-semibold tracking-wider text-[var(--color-accent)] uppercase">Design Logic</span></div>
        <p className="text-sm leading-relaxed">{m.design_logic}</p>
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.15 }} className="flex flex-wrap gap-2 mb-6">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.2 }} className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border-light)] p-5 mb-6" style={{ boxShadow: '0 20px 40px -15px rgba(0,0,0,0.03)' }}>
      ...
      {(error || sessionError) && (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-[var(--color-error)] bg-[var(--color-error-light)] px-4 py-3 rounded-xl mb-4 flex items-center gap-2">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-4">
            <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border-light)] p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowRegen(false)} className="p-1 rounded-lg hover:bg-[var(--color-warm-100)]"><X size={16} weight="bold" className="text-[var(--color-muted)]" /></button>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {REASONS.map(r => (
                  <button key={r.value} onClick={() => setReason(r.value)}
                    className={`py-2 px-3 rounded-xl text-xs font-medium border transition-all active:scale-[0.98] ${reason === r.value ? 'border-[var(--color-accent)] bg-[var(--color-accent-light)] text-[var(--color-accent)]' : 'border-[var(--color-border-light)] hover:border-[var(--color-border)]'}`}>{r.label}</button>
                ))}
              </div>
              <button onClick={handleRegen} disabled={!reason || regenLoading} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-[var(--color-foreground)] text-[var(--color-background)] hover:opacity-90 disabled:opacity-40 active:scale-[0.98] transition-all">
