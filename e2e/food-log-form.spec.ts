...
/** Navigate to home, wait for FoodLogForm to render (State A). */
async function goToHomeFoodLog(page: Page) {
  await page.goto('/noa/home');
  await expect(page.locator('text=用餐怎么样？')).toBeVisible({ timeout: 8_000 });
}
...
// ─── Tests: 首页 FoodLogForm (State A 内联) ──────────────────────────────────

test.describe('首页内联 FoodLogForm', () => {
  test.beforeEach(async ({ page }) => {
    await setAuthToken(page);
    await mockHomeStatusNoBook(page);
    await mockFoodLogSuccess(page);
...
// ─── Tests: 首页弹窗 FoodLogForm ─────────────────────────────────────────────

test.describe('首页弹窗 FoodLogForm', () => {
  test.beforeEach(async ({ page }) => {
    await setAuthToken(page);
